...
def MOEMS_generate_by_image(filepath, filename, source="LCP", phase="M", period=5e-6, h_Pt=50e-9, h_VO2=0.16e-6, h_Cr=30e-9, h_Au=90e-9, h_air=1e-6):
    ...
    pixels = np.array([[0 if img_arr[i,j] < np.mean(img_arr) else np.max(img_arr) for j in range(img_arr.shape[1])] for i in range(img_arr.shape[0])])
    ...
    # Start up FDTD Model
    fdtd = pyFDTD.Model(filepath, filename+f"_{source}"+f"_{phase}", show=True)
